...
class Mask(object):
    ...
    def setHelicoidal(self, R, x=0, y=0, rotation=True):
        ...
        self.img = pm.helMask(self.m, self.n, R, u=x, v=y, rotation=rotation)
        self.two_PiToUInt8()
        return

    def setGaussian(self, radius, sigma, x=0, y=0):
        """replaces self.img with a gaussian shape. radius corresponds to the radius
        of the desired phase mask in pixels"""
        """print("setGaussian img called")"""
        "self.img=pm.gaussian(self.m,self.n,radius,sigma,x,y)"
        self.img = pm.createGsMask(self.m, self.n, radius, sigma, x, y)
        # Conversion into a uint8 image
        self.two_PiToUInt8()
        """print("setGaussian img finished")"""
        return
    ...
    def setHalf(self, R, x=0, y=0, rotang=0):
    ...
    def setQuad(self, R, x=0, y=0, rotang=0):
    ...
    def setHex(self, R, x=0, y=0, rotang=0):
    ...
    def setSplit(self, R, x=0, y=0, rotang=0):
    ...
    def setWhite(self, r, xcenter, ycenter):
        ...
        self.img = np.ones((self.m, self.n), dtype=np.uint8)
        return
...
class DoubleMask(EvMask):
    ...
    def moveLeft(self, x, y):
        """Moves the left centre from x,y"""
        (m, n) = self.left.img.shape
        # leftCenter represents the actual position of the left mask, in the coordinates
        # understood by Python. self.left_center represents the position of the mask
...
class Aberrations(DoubleMask):
    """class creating a mask containing a helix on the left part of the chip and a top hat
    on the right part. R corresponds to the radius of each of the masks in pixels"""
    def __init__(self, m, n, lbd, R, sigma, DaberrationFactors, THaberrationFactors, left_pos=(0, 0), right_pos=(0, 0), rotation=True):
        DoubleMask.__init__(self, m, n, lbd, left_center=left_pos, right_center=right_pos)
        self.R = R
        if self.R > min(m // 2, n // 2):
            print("R out of range")
        ...
        self.left.aberrations(left_pos[0], left_pos[1], R, DaberrationFactors)
...
class Half(DoubleMask):
    """class creating a mask containing a helix on the left part of the chip and a top hat
    on the right part. R corresponds to the radius of each of the masks in pixels"""
    def __init__(self, m, n, lbd, R, sigma, left_pos=(0, 0), right_pos=(0, 0), rotang=0):
        DoubleMask.__init__(self, m, n, lbd, left_center=left_pos, right_center=right_pos)
        self.R = R
        ...
        if self.R > min(m // 2, n // 2):
            print("R out of range")
            self.R = min(m // 2, n // 2)
        self.left.setHalf(R, left_pos[0], left_pos[1], rotang)
        self.right.setHalf(R, right_pos[0], right_pos[1], rotang)
        self.update()
...
class Quad(DoubleMask):
    ...
    def __init__(self, m, n, lbd, R, sigma, left_pos=(0, 0), right_pos=(0, 0), rotang=0):
        ...
        if self.R > min(m // 2, n // 2):
            print("R out of range")
            self.R = min(m // 2, n // 2)
        self.left.setQuad(R, left_pos[0], left_pos[1], rotang)
        self.right.setQuad(R, right_pos[0], right_pos[1], rotang)
        self.update()
...
class Hex(DoubleMask):
    ...
    def __init__(self, m, n, lbd, R, sigma, left_pos=(0, 0), right_pos=(0, 0), rotang=0):
        ...
        if self.R > min(m // 2, n // 2):
            print("R out of range")
            self.R = min(m // 2, n // 2)
        self.left.setHex(R, left_pos[0], left_pos[1], rotang)
        self.right.setHex(R, right_pos[0], right_pos[1], rotang)
        self.update()
...
class Split(DoubleMask):
    ...
    def __init__(self, m, n, lbd, R, sigma, left_pos=(0, 0), right_pos=(0, 0), rotang=0):
        ...
        if self.R > min(m // 2, n // 2):
            print("R out of range")
            self.R = min(m // 2, n // 2)
        self.left.setSplit(R, left_pos[0], left_pos[1], rotang)
        self.right.setSplit(R, right_pos[0], right_pos[1], rotang)
        self.update()
